package com.example.BookkeepingBook.account;
        ...
        btn_acc_save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (TextUtils.isEmpty(etAccMoney)) {
                    ...
                    Toast.makeText(AccountAdd.this, spAssNmae, Toast.LENGTH_SHORT).show();
                    // boolean assets = assetsDao.findByAssName(spAssNmae,
                    // LoginActivity.getmUsername());// 从资产获取资产名称下的多少钱
                    Assets assets = assetsDao.findByAssName(spAssNmae,
                            LoginActivity.getmUsername());// 从资产获取资产名称下的多少钱
                    if (assets != null) {
                        Double s1 = assets.getAssetsMoney() + s;// 资产的和这里的相加
                        assetsDao.updateAssets(assets.getId(), assets.getAssetsName(),
                                assets.getAssetsType(), s1,
                                assets.getRemarks());
                    }
                    accountDao.addAccount(s, spAccType, spAccPayType, spAssNmae, time,
                            etAccRemarks,
