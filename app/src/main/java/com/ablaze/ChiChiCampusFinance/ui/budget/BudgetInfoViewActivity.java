 ...
 * 菜单栏2
 */
public class BudgetInfoViewActivity extends Activity {
    ...
    public BudgetInfoViewActivity(Activity context) {
        mContext = context;
        // 为以后将Layout转换为view时用
    ...
    private void initView() {
        ...
        btn_bud.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (readLoginStatus()) {
                    // 跳转到详情界面
                    Intent intent = new Intent(mContext, BudgetListActivity.class);
                    mContext.startActivityForResult(intent, 1);
                } else {
                    Toast.makeText(mContext, "您还未登录，请先登录", Toast.LENGTH_SHORT).show();
                }
